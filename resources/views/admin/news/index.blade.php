                ...
                <tbody>
                    @forelse($newsList as $news)
                        <tr>
                            ...
                            <td>{{ $news->status }}</td>
                            <td>{{ $news->author }}</td>
                            <td>@if($news->updated_at) {{ $news->updated_at->format('d-m-Y H:i') }} @else {{ now()->format('d-m-Y H:i') }} @endif</td>
                            <td>
                                <a href="{{ route('admin.news.edit', ['news' => $news->id]) }}" style="font-size: 12px">Edit</a> &nbsp;
                                <a href="javascript:;" rel="{{ $news->id }}" class="delete" style="font-size: 12px; color: red">Del.</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
